    ...
    ILogger log)
{
    ...
    try
    {
        ...
        // Initialize Spell Slots if caster class 
...
{
    // List of classes that normally get spellcasting at level 1
    ...
    var thirdCasters = new List<string> { "fighter", "rogue" }; // Eldritch Knight, Arcane Trickster — optional for later
    ...
    // Very basic initial slot chart: real one can be plugged later
    character.SpellSlots = new Dictionary<int, int>();

    if (effectiveLevel >= 1)
        character.SpellSlots[1] = 2;
    if (effectiveLevel >= 3)
        character.SpellSlots[2] = 2;
    if (effectiveLevel >= 5)
        character.SpellSlots[3] = 2;
    if (effectiveLevel >= 7)
        character.SpellSlots[4] = 1;
    if (effectiveLevel >= 9)
        character.SpellSlots[5] = 1;
}
